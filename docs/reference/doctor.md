...
The `doctor` command did not exist in earlier versions.
:::

The `doctor` command inspects the machine it is run on and reports what would stop Butler Sheet
Icons working, with the steps that fix each problem.

## What it is

It is the general form of [`browser check`](/reference/browser#check). Where `browser check`
answers one question — *can this server take screenshots?* — `doctor` runs every check Butler Sheet
Icons has, so it is the command to reach for when a run has failed and you do not yet have a theory
about why.
...
- **It makes no network requests** unless you pass `--allow-network`. On a server with no internet
  ...
  the network are skipped instead, and the report says so.
...
## What it checks {#doctor-what-it-checks}
...
today reports that nothing was checked, and **fails**; see
[Areas with no checks yet](#doctor-empty-areas) below.
...
## A healthy server {#doctor-healthy-example}
...
## A server that cannot work {#doctor-failure-example}
...
## The exit code {#doctor-exit-code}
...
```powershell [PowerShell]
.\butler-sheet-icons.exe doctor check
if ($LASTEXITCODE -ne 0) {
    Write-Error "Butler Sheet Icons is not ready on this server"
    exit 1
}
```
...
## Checking one area only {#doctor-areas}
...
### Areas with no checks yet {#doctor-empty-areas}
...
## Machine-readable output {#doctor-json}
...
```json
{
  ...
  "toolVersion": "(the Butler Sheet Icons version that produced the report)",
...
### The JSON is redacted before it is written {#doctor-json-redaction}

Because this document is designed to be shared, every part of it — the details, the facts, the
evidence and the suggested commands — is passed through Butler Sheet Icons'
[secret redaction](/reference/log-redaction) before anything is written. Passwords, API keys,
bearer tokens and credentials embedded in URLs are replaced with `[REDACTED]`:
...
## Checks that need the network {#doctor-allow-network}
...
## When to run it {#doctor-when-to-run}
...
- **Whenever a run fails and you do not know why** — and attach `--outputformat json` output to the
  issue if you end up opening one. See also the [Troubleshooting](/guide/troubleshooting) page for
  symptom-first help.
