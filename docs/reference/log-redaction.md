...
- [Crash dump files](/guide/advanced/crash-dumps) — what is in a crash dump, where the files are written, and how to share them with support.
- [Environment variables](/guide/concepts/environment-variables) — how to set `BSI_LOG_LEVEL` and other environment variables on Windows, macOS, and Linux.
- [Security](/reference/security) — signing, disclosure, and other security topics.
