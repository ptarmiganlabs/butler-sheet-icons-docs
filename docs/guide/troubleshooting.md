...
### Every sheet's thumbnail is a picture of the login screen {#thumbnails-show-the-login-screen}

::: warning Requires BSI 5.0.0 or later
This is the description of a problem that earlier versions did not catch. Before 5.0.0 a failed sign-in was not detected: Butler Sheet Icons photographed the login page once per sheet, uploaded those images as thumbnails, and reported the run as **successful**. If you are on an earlier version and see login-screen thumbnails, that is the cause.
:::

**Symptom:** an app's sheets all show a picture of a Qlik Sense login form instead of a miniature of the sheet, and the run that produced them reported success.

From 5.0.0 this cannot happen: Butler Sheet Icons checks that the sign-in actually worked, and **stops before it uploads anything** if the browser is still on the login page. The run fails at startup with a message naming what it expected. On Qlik Sense Enterprise on Windows:

```text
QSEOW APP: not authenticated after submitting the credentials and waiting for navigation. The login form's username field (#username-input) is still on the page, so the browser is on the login screen. Continuing would capture that screen as though it were a sheet. Check the username and password supplied to BSI, and that the login page is the form this strategy expects.
```

On Qlik Sense Cloud the message is the same, naming that platform's field (`[id="1-email"]`) instead.
...
| Password change or account lockout                    | A common cause of a job that worked for months and then stopped                                      |
| The account cannot open the app in a browser          | Sign in manually as that same user and open the app                                                  |
| The virtual proxy expects something other than a form | A proxy set up for header or certificate authentication does not present the form BSI types into — see [QSEoW Authentication Problems](#qseow-authentication-problems) |

**If some apps already have login-screen thumbnails**, they were produced by a run that failed this way before 5.0.0. Fix the sign-in problem and run Butler Sheet Icons again — the thumbnails are simply overwritten, with nothing to clean up first. There is no way to find them automatically after the fact, so it is worth checking any app whose thumbnails were last generated by a run nobody watched.

#### If you use `--skip-login` on Qlik Sense Cloud {#skip-login-not-signed-in}

`--skip-login` tells Butler Sheet Icons that the browser profile it uses is already signed in, so it should not enter credentials. If that is not true — most often because the session it relied on has expired — the result before 5.0.0 was the same silent failure. That case is now checked too:

```text
CLOUD APP: not authenticated after opening the app with --skip-login. The login form's username field ([id="1-email"]) is still on the page, so the browser is on the login screen. Continuing would capture that screen as though it were a sheet. The session --skip-login relies on is not signed in. Sign in to Qlik Sense Cloud in the browser profile BSI uses, or drop --skip-login and supply credentials.
```

Sign in again in the browser profile Butler Sheet Icons uses, or stop passing `--skip-login` and let BSI sign in itself.
...
- Stuck on login page, or [thumbnails that show the login screen](#thumbnails-show-the-login-screen)
...
- Connection timeouts
- A sign-in that leaves the browser on the login page — see [thumbnails that show the login screen](#thumbnails-show-the-login-screen)
